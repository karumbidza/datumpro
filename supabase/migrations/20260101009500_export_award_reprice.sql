...
-- Reassignment cleanup must not wipe BOQ-generated lines. The trigger's intent
-- (20260101003100) is to clear a PREVIOUS assignee's personal plan; lines that
-- came from the bill (boq_item_id set) are project scope — they survive
-- (re)assignment so the generate→assign flow and award repricing keep the plan.
