-- ─────────────────────────────────────────────────────────────────────────────
-- DatumPro — BOQ programme, part 2: engine
-- (spec 2026-08-25-boq-programme-design)
--
-- • generate_tasks_from_boq: rolls line duration_days up into
--   tasks.agreed_duration_days and copies boq_section_deps → task_dependencies.
-- • export_award_to_project: same roll-up from the winner's bid days
--   (falling back to budget days) in both generate and reprice modes.
-- • schedule_boq_tasks: forward pass — no-predecessor tasks start on the given
--   date (concurrent), successors the working day after their latest
--   predecessor ends, end = start + agreed duration in working days on the
--   project calendar. Writes planned_start/planned_end/due_date; only 'todo'
--   tasks are rewritten, started/done tasks are frozen inputs.
-- ─────────────────────────────────────────────────────────────────────────────
...
declare
  ...
  v_task uuid; v_task_total bigint; v_task_days int;
...
begin
  ...
  loop
    ...
    -- Agreed SLA duration = sum of the section's line durations (null if none set).
    select sum(i.duration_days) into v_task_days
      from public.boq_items i where i.section_id = v_section.id;

    insert into public.tasks
      (org_id, project_id, title, boq_section_id, requires_photo_on_complete, agreed_duration_days, created_at)
      values (v_org, p_project_id, v_section.sec_no || '. ' || v_section.name, v_section.id, false,
              v_task_days, clock_timestamp())
  ...
  -- Section links become task dependencies (the shipped start-gate enforces them).
  ...
  join public.boq_sections s on s.id = d.section_id and s.boq_id = p_boq_id
  join public.tasks ts on ts.boq_section_id = d.section_id    and ts.project_id = p_project_id
  join public.tasks tp on tp.boq_section_id = d.depends_on_id and tp.project_id = p_project_id
...
-- ── The scheduler ────────────────────────────────────────────────────────────
create or replace function public.schedule_boq_tasks(
  p_project_id uuid,
  p_boq_id uuid,
  p_start_date date
) returns jsonb
language plpgsql security definer set search_path = '' as $$
declare
  v_org uuid; v_cal uuid;
  v_total int; v_done int := 0; v_pass int := 0;
  v_frozen int := 0; v_scheduled int := 0;
  v_missing jsonb := '[]'::jsonb;
  v_project_end date := null;
  r record;
  v_start date; v_end date; v_pred_end date; v_lag int;
begin
  select org_id, calendar_id into v_org, v_cal from public.projects where id = p_project_id;
  if v_org is null then raise exception 'project not found'; end if;
  if not (coalesce(public.is_org_admin(v_org), false) or coalesce(public.org_role(v_org), '') = 'pm') then
    raise exception 'not authorised';
  end if;
  if p_start_date is null then raise exception 'start date required'; end if;
  -- Matches the tasks_no_backdated_start trigger — fail fast with a clear message.
  if p_start_date < current_date then raise exception 'the start date can''t be in the past'; end if;
  if not exists (select 1 from public.boqs where id = p_boq_id and org_id = v_org) then
    raise exception 'BOQ not found in this organisation';
  end if;

  -- Working set: this bill's generated tasks in this project. done_at holds the
  -- computed end so successors of frozen tasks chain off real dates.
  create temp table _sched on commit drop as
    select t.id, t.agreed_duration_days as dur,
           (t.status <> 'todo') as frozen,
           t.planned_start_date as ps, t.planned_end_date as pe,
           false as placed
    from public.tasks t
    join public.boq_sections s on s.id = t.boq_section_id
    where t.project_id = p_project_id and s.boq_id = p_boq_id;

  select count(*) into v_total from _sched;
  if v_total = 0 then raise exception 'no generated tasks to schedule'; end if;

  -- Frozen tasks keep their dates and count as placed immediately.
  update _sched set placed = true where frozen;
  select count(*) into v_frozen from _sched where frozen;
  v_done := v_frozen;

  while v_done < v_total loop
    v_pass := v_pass + 1;
    if v_pass > v_total + 1 then
      raise exception 'dependency cycle detected — fix the section links and retry';
    end if;

    for r in
      select st.id, st.dur from _sched st
      where not st.placed
        and not exists (
          select 1 from public.task_dependencies d
          join _sched p on p.id = d.predecessor_id
          where d.successor_id = st.id and not p.placed
        )
    loop
      -- Latest predecessor end (within the set; a frozen task with no dates, or a
      -- predecessor outside the bill, contributes nothing).
      select max(coalesce(p.pe, p.ps) + coalesce(d.lag_days, 0)) into v_pred_end
      from public.task_dependencies d
      join _sched p on p.id = d.predecessor_id
      where d.successor_id = r.id;

      if v_pred_end is null then
        v_start := p_start_date;
      elsif v_cal is not null then
        v_start := public.add_working_days(v_pred_end, 1, v_cal, p_project_id);
      else
        v_start := v_pred_end + 1;
      end if;

      if r.dur is null then
        v_end := null;
        v_missing := v_missing || to_jsonb((select title from public.tasks where id = r.id));
      elsif v_cal is not null then
        v_end := public.add_working_days(v_start, greatest(r.dur, 1) - 1, v_cal, p_project_id);
      else
        v_end := v_start + (greatest(r.dur, 1) - 1);
      end if;

      update public.tasks
         set planned_start_date = v_start, planned_end_date = v_end, due_date = v_end
       where id = r.id;
      update _sched set placed = true, ps = v_start, pe = v_end where id = r.id;
      v_scheduled := v_scheduled + 1;
      v_done := v_done + 1;
      if v_end is not null and (v_project_end is null or v_end > v_project_end) then
        v_project_end := v_end;
      end if;
    end loop;
  end loop;
  ...
  return jsonb_build_object(
    'scheduled', v_scheduled, 'frozen', v_frozen,
    'missing_duration', v_missing, 'project_end', v_project_end);
end $$;
revoke all on function public.schedule_boq_tasks(uuid, uuid, date) from public;
grant execute on function public.schedule_boq_tasks(uuid, uuid, date) to authenticated;
