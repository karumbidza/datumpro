    ...
    <>
      <div className="flex items-center justify-between border-b border-zinc-200 px-4 py-3.5 dark:border-zinc-800">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[11px] text-zinc-500 dark:text-zinc-400">{members.length}</span>
          {onClose && (
            <button
              type="button"
              onClick={onClose}
              aria-label="Close people"
              className="-mr-1 flex h-8 w-8 items-center justify-center rounded-lg text-zinc-500 dark:text-zinc-400 hover:bg-zinc-100 lg:hidden dark:hover:bg-zinc-800"
            >
              <X size={16} />
            </button>
          )}
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {online.length > 0 && (
          <>
            <p className="px-2 pb-1 pt-2 text-[10px] font-medium uppercase tracking-[0.05em] text-zinc-500 dark:text-zinc-400">
        ...
        {offline.length > 0 && (
          <>
            <p className="px-2 pb-1 pt-3.5 text-[10px] font-medium uppercase tracking-[0.05em] text-zinc-500 dark:text-zinc-400">
        ...
        {members.length === 0 && (
          <p className="px-2 py-8 text-center text-sm text-zinc-500 dark:text-zinc-400">No members yet.</p>
        )}
      </div>
    </>
  );
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="block truncate text-sm font-medium text-zinc-900 dark:text-zinc-100">
          {member.name}
          {isMe && <span className="text-zinc-500 dark:text-zinc-400"> (You)</span>}
        </span>
        <span className="block truncate text-[11px] text-zinc-500 dark:text-zinc-400">{sub}</span>
      </span>
      ...
      <div className="flex items-center gap-2 border-b border-zinc-200 px-3 py-3 dark:border-zinc-800">
        ...
        {onClose && (
          <button
            ...
            onClick={onClose}
            aria-label="Close people"
            className="ml-auto flex h-8 w-8 items-center justify-center rounded-lg text-zinc-500 dark:text-zinc-400 hover:bg-zinc-100 lg:hidden dark:hover:bg-zinc-800"
          >
            <X size={16} />
          </button>
        )}
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="flex flex-col items-center gap-2 border-b border-zinc-100 px-5 py-6 dark:border-zinc-800">
          ...
          <p className="text-base font-semibold text-zinc-900 dark:text-white">
            {member.name}
            {member.userId === currentUserId && <span className="text-zinc-500 dark:text-zinc-400"> (You)</span>}
          </p>
          <p className={`text-xs ${online ? 'text-green-600 dark:text-green-400' : 'text-zinc-500 dark:text-zinc-400'}`}>
        ...
        {(member.phone || member.email) && (
          <div className="border-b border-zinc-100 px-5 py-4 dark:border-zinc-800">
            <p className="mb-2 text-[10px] font-medium uppercase tracking-[0.05em] text-zinc-500 dark:text-zinc-400">Contact</p>
            {member.phone && (
              <p className="flex items-center gap-2 py-1 text-sm text-zinc-700 dark:text-zinc-300">
                <Phone size={15} className="flex-shrink-0 text-zinc-500 dark:text-zinc-400" /> {member.phone}
              </p>
            )}
            {member.email && (
              <p className="flex items-center gap-2 py-1 text-sm text-zinc-700 dark:text-zinc-300">
                <Mail size={15} className="flex-shrink-0 text-zinc-500 dark:text-zinc-400" />
        ...
        {/* On this project */}
        <div className="border-b border-zinc-100 px-5 py-4 dark:border-zinc-800">
          <p className="mb-2 text-[10px] font-medium uppercase tracking-[0.05em] text-zinc-500 dark:text-zinc-400">On this project</p>
        ...
        <div className="px-5 py-4">
          <p className="mb-3 text-[10px] font-medium uppercase tracking-[0.05em] text-zinc-500 dark:text-zinc-400">Recent activity</p>
          {activity === null ? (
            <p className="text-xs text-zinc-500 dark:text-zinc-400">Loading…</p>
          ) : activity.length === 0 ? (
            <p className="text-xs text-zinc-500 dark:text-zinc-400">No recent activity on this project.</p>
          ) : (
            <ul className="flex flex-col gap-3 border-l border-zinc-200 pl-3.5 dark:border-zinc-800">
              {activity.map((a) => (
                <li key={a.id} className="relative">
                  ...
                  <p className="text-[11px] text-zinc-500 dark:text-zinc-400">{relTimestamp(a.at)}</p>
                </li>
              ))}
            </ul>
          )}
