...
export function computeNav(
  ...
  if (activeProject) {
    ...
    // Site diary, Snagging, Drawings, RFIs and Transmittals now live in the Project
    // Chat rail's action row (not the sidebar). Variations stays here.
    const items: NavItem[] = [
  ...
  return [
    ...
    ...(memberType === 'contractor'
      ...
      : memberType === 'client' || memberType === 'viewer'
        ? []
        : // The bills-and-tenders library is an owner/admin surface. PMs are
          // project-scoped: they reach their project's BOQ from inside the
          // project, not the org-wide library.
          canManageMembers
