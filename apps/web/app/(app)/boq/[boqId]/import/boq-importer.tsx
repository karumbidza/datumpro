...
type Row = { kind: Kind; itemNo: string; description: string; unit: string; qty: number; rate: number };
...
const hasVal = (c: Cell | undefined): boolean => str(c) !== '';
...
function guess(grid: Grid): { headerSkip: number; roles: Role[] } {
  ...
  if (headerRow >= 0) {
    ...
    for (let c = 0; c < cols; c++) {
      ...
      if (!h) continue;
      if (/qty|quant/.test(h)) roles[c] = 'qty';
      else if (/rate|price/.test(h)) roles[c] = 'rate';
      else if (/amount|total|value/.test(h)) roles[c] = 'amount';
  ...
  return { headerSkip: headerRow >= 0 ? headerRow + 1 : 0, roles };
}
...
export function BoqImporter({ boqId, currency }: { boqId: string; currency: string }) {
  ...
  async function onFile(file: File) {
    ...
    try {
      ...
      setSheets(parsed);
      setSheetIndex(0);
      const g = guess(first.grid);
      setHeaderSkip(g.headerSkip);
      setRoles(g.roles);
      setSectionMode(g.roles.includes('section') ? 'column' : 'auto');
      setStep('map');
    } catch {
  ...
  function buildReview() {
    ...
    const built: Row[] = grid.slice(headerSkip).map((r) => {
      ...
      const hasQ = hasVal(get(r, 'qty'));
      const hasR = hasVal(get(r, 'rate'));
      const hasD = description !== '';
      let kind: Kind;
      if (sectionMode === 'column') kind = sectionCell !== '' ? 'section' : hasD || hasQ || hasR ? 'item' : 'skip';
      else kind = hasD && !hasQ && !hasR ? 'section' : hasD || hasQ || hasR ? 'item' : 'skip';
      const desc = kind === 'section' && sectionCell ? sectionCell : description;
      return { kind, itemNo: str(get(r, 'item_no')), description: desc, unit, qty, rate };
    });
    // Drop rows that carry nothing at all.
    setRows(built.filter((r) => !(r.description === '' && r.qty === 0 && r.rate === 0)));
    setStep('review');
  }
  ...
  const totals = useMemo(() => {
    ...
    for (const r of rows) {
      if (r.kind === 'skip') skipped++;
        ...
        items++;
        cents += Math.round(r.qty * r.rate * 100);
      }
    }
  ...
  function toSections(): ImportSection[] {
    ...
    for (const r of rows) {
      ...
      if (r.kind === 'section') {
        ...
        cur.items.push({ description: r.description, uom: r.unit || null, qty: r.qty, rateCents: Math.round(r.rate * 100) });
      }
    }
    ...
    return (
      <div className="mt-6">
        ...
        <label className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-zinc-300 bg-zinc-50 px-6 py-14 text-center transition hover:border-brand-500 hover:bg-brand-50/40 dark:border-zinc-700 dark:bg-zinc-900/40">
          ...
          <span className="text-xs text-zinc-500 dark:text-zinc-400">.xlsx · .xls · .csv — any column layout</span>
          <input
            type="file"
  ...
  // ── MAP ─────────────────────────────────────────────────────────────────────
  if (step === 'map') {
    const previewRows = grid.slice(0, Math.min(grid.length, headerSkip + 5));
    return (
      <div className="mt-6 space-y-5">
        ...
        <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
          ...
          <label className="flex items-center gap-2 text-sm">
            ...
            <select value={sectionMode} onChange={(e) => setSectionMode(e.target.value as 'auto' | 'column')} className={inputCompactClass}>
              <option value="auto">Auto — a row with no qty/rate is a heading</option>
        ...
        <p className="text-sm text-zinc-500 dark:text-zinc-400">
          Set what each column is. Greyed rows above the line are skipped as headers.
        </p>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" onClick={buildReview} disabled={!roles.includes('description')}>
            Continue to review →
          </Button>
  ...
  return (
    <div className="mt-6 space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-zinc-500 dark:text-zinc-400">
          Fix anything that landed wrong — flip a row between <b>Section</b>, <b>Item</b> and <b>Skip</b>, or edit a value. Then import.
        </p>
              ...
              return (
                <tr key={i} className={isSection ? 'bg-zinc-50 dark:bg-zinc-900/50' : isSkip ? 'opacity-45' : ''}>
                  ...
                  <td className="border-b border-zinc-200 px-2 py-1.5 text-right font-mono tabular-nums dark:border-zinc-800">
                    {isSection || isSkip ? '' : fmtMoney(Math.round(r.qty * r.rate * 100), currency)}
                  </td>
                </tr>
              );
            })}
