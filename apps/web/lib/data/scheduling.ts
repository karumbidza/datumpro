...
/** Weight = duration for now (agreed contractor cost will replace it once the
 *  commitment flow lands, giving true Earned Value). */
...
export async function getProjectSchedule(projectId: string): Promise<ProjectSchedule | null> {
  ...
  const { data: taskData } = await supabase
    .from('tasks')
    .select('id, status, planned_start_date, planned_end_date, due_date')
